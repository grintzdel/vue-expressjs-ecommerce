  ...
  <div class="p-6" @click="activeDropdown = null">
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p class="text-sm font-body text-font-secondary mt-1">Gérez les tags de vos produits</p>
      </div>
    ...
    <div class="bg-white rounded-xl border border-border-light overflow-hidden">
      ...
      <div v-if="filteredTags.length === 0" class="px-5 py-10 text-center font-body text-sm text-font-tertiary">
        Aucun tag trouvé
      </div>
      ...
      >
        ...
        <div class="w-[70px] flex justify-center relative">
          <button class="p-1 text-font-tertiary hover:text-font-primary" @click.stop="activeDropdown = activeDropdown === tag.id ? null : tag.id">
          ...
          <div v-if="activeDropdown === tag.id" class="absolute right-0 top-8 z-20 bg-white border border-border-light rounded-lg shadow-lg py-1.5 w-[170px]" @click.stop>
            <button class="block w-full text-left px-4 py-2 text-sm font-body text-font-primary hover:bg-bg-primary transition" @click="openEdit(tag)">Modifier</button>
            <button class="block w-full text-left px-4 py-2 text-sm font-body text-[#C94444] hover:bg-bg-primary transition" @click="remove(tag.id)">Supprimer</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div class="relative bg-white rounded-xl shadow-xl w-full max-w-[560px] p-6">
        <h2 class="font-heading text-xl font-bold text-font-primary mb-6">{{ editingId ? 'Modifier' : 'Ajouter' }} un tag</h2>
        <div class="space-y-4">
          <div>
