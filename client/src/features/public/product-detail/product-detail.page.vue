...
const productTags = computed(() => {
  if (!product.value || !tags.value) return []
  return tags.value.filter(t => (product.value as any).tagIds?.includes(t.id))
})

const productSkinTypes = computed(() => {
  if (!product.value || !skinTypes.value) return []
  return skinTypes.value.filter(s => (product.value as any).skinTypeIds?.includes(s.id))
})
...
const relatedProducts = computed(() => {
  ...
  if (related.length < 4) {
    const remaining = allProducts.value
      .filter(p => p.id !== product.value!.id && !related.find(r => r.id === p.id))
...
const ingredientsList = computed(() => {
  const ingredients = (product.value as any)?.ingredients
...
function formatPrice(p: ProductDomainModel.ProductOverviewDto): string {
...
function getProductImage(p: ProductDomainModel.ProductOverviewDto): string | null {
...
function generateSKU(s: string): string {
  const parts = s.split('-')
  const prefix = 'VLV'
...
<template>
  <!-- Loading -->
  <div v-if="isLoading" class="flex items-center justify-center py-4xl">
    <p class="font-body text-font-tertiary">Loading...</p>
  </div>

  <!-- Error -->
  <div v-else-if="isError" class="flex flex-col items-center justify-center py-4xl gap-md">
    ...
    <section class="px-lg lg:px-[80px] py-xl">
      <div class="flex flex-col lg:flex-row gap-3xl">
        ...
        <div class="flex flex-col-reverse lg:flex-row gap-md lg:w-[640px] flex-shrink-0">
          <!-- Thumbnails -->
          <div v-if="product.images && product.images.length > 1" class="flex lg:flex-col gap-2 overflow-x-auto lg:overflow-visible">
            <button
        ...
        <div class="flex flex-col gap-lg flex-1">
          ...
          <div class="flex items-center gap-3">
            <span class="text-accent-green text-body">{{ renderStars((product as any).rating || 4) }}</span>
            <span class="font-body text-sm font-semibold text-font-primary">{{ ((product as any).rating || 4).toFixed(1) }} / 5</span>
      ...
      <div class="py-xl">
        ...
        <div v-else-if="activeTab === 'howToUse'" class="max-w-3xl">
          ...
          <p class="font-body text-[15px] leading-[1.7] text-font-secondary whitespace-pre-line">
            {{ (product as any).howToUse || 'Usage instructions coming soon.' }}
          </p>
        </div>
        ...
        <div v-else-if="activeTab === 'reviews'" class="max-w-3xl">
          ...
          <div class="flex items-center gap-3 mb-lg">
            <span class="text-accent-green text-subsection">{{ renderStars((product as any).rating || 4) }}</span>
            <span class="font-body text-body-lg font-semibold text-font-primary">{{ ((product as any).rating || 4).toFixed(1) }} out of 5</span>
          </div>
        ...
        <div v-else-if="activeTab === 'shipping'" class="max-w-3xl">
          ...
          <p class="font-body text-[15px] leading-[1.7] text-font-secondary whitespace-pre-line">
            {{ (product as any).shippingInfo || 'Shipping information coming soon.' }}
          </p>
        </div>
      </div>
    </section>
          ...
          <div class="flex flex-col gap-3 p-md pb-5">
            <div class="flex items-center justify-between">
              ...
              <div class="flex items-center gap-1 flex-shrink-0">
                <span class="text-accent-green text-xs">{{ renderStars((rp as any).rating || 4) }}</span>
              </div>
            </div>
