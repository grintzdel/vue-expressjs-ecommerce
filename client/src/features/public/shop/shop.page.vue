...
// Update price bounds when products load
watch(allProductsData, (prods) => {
...
const filteredProducts = computed(() => {
  let result = allProductsData.value ?? []

  if (selectedCategory.value) {
    result = result.filter(p => p.categoryId === selectedCategory.value)
  }
  if (selectedSkinType.value) {
    result = result.filter(p => (p as any).skinTypeIds?.includes(selectedSkinType.value!))
  }
  if (selectedTag.value) {
    result = result.filter(p => (p as any).tagIds?.includes(selectedTag.value!))
  }
              ...
              <div class="flex flex-col gap-3 p-md pb-5">
                ...
                <div class="flex items-center justify-between">
                  ...
                  <div class="flex items-center gap-1 flex-shrink-0">
                    <span class="text-accent-green text-xs">{{ renderStars((product as any).rating || 4) }}</span>
                    <span class="font-body text-[11px] text-font-tertiary">({{ (product as any).reviewCount || 0 }})</span>
                  </div>
                </div>
