...
import { useGetFeaturedProducts } from '@/modules/product/ui/hooks/queries/query/use-get-featured-products'
import { useGetFeaturedTestimonials } from '@/modules/testimonial/ui/hooks/queries/query/use-get-featured-testimonials'
import { useGetPressLogos } from '@/modules/press-logo/ui/hooks/queries/query/use-get-press-logos'
import { useSubscribe } from '@/modules/newsletter/ui/hooks/queries/mutation/use-subscribe'
...
const { data: featuredProductsData } = useGetFeaturedProducts()
const { data: featuredTestimonials } = useGetFeaturedTestimonials()
const { data: pressLogos } = useGetPressLogos()
...
const featuredTestimonial = computed(() => {
  return featuredTestimonials.value?.[0] ?? null
})

const featuredProducts = computed(() => {
  const featured = featuredProductsData.value
...
const filteredRoutineProducts = computed(() => {
  const all = allProducts.value ?? []
  const cats = categories.value ?? []
  let filtered = all

  if (activeCategory.value !== 'all') {
    const cat = cats.find(c => c.slug === activeCategory.value)
    if (cat) filtered = filtered.filter(p => p.categoryId === cat.id)
  }
  // activeSkinType filtering would require skinTypeIds on product
  return filtered.slice(0, 8)
})

function getProductImage(product: ProductDomainModel.ProductOverviewDto): string | null {
...
function formatPrice(product: ProductDomainModel.ProductOverviewDto): string {
  ...
  <div>
    ...
    <section class="bg-bg-primary md:bg-white py-[40px] md:py-[80px] px-[20px] md:px-3xl">
      <div class="max-w-[1200px] mx-auto">
        ...
        <div v-if="pressLogos && pressLogos.length > 0" class="border-t border-border-light pt-2xl">
